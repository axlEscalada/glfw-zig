...
const VideoDecoder = struct {
    ...
    fn makeDecoderCtx(fmt_ctx: *c.AVFormatContext) VideoDecoderError!*c.AVCodecContext {
        if (c.avformat_find_stream_info(fmt_ctx, null) < 0) {
            std.log.err("Failed to find stream", .{});
            return error.InvalidData;
        }
        ...
        var decoder_ctx = c.avcodec_alloc_context3(decoder) orelse {
...
fn imgToTexture(image: Image) !c.GLuint {
    ...
    c.glTexImage2D(c.GL_TEXTURE_2D, 0, c.GL_RGBA, @intCast(image.stride), @intCast(image.height), 0, c.GL_RED, c.GL_UNSIGNED_BYTE, image.data.ptr);
...
pub fn main() !void {
    ...
    const frag_shader_source: [*c]const u8 =
        ...
        \\  frag_coord.y *= -1;
        \\  float luma = texture(tex, frag_coord).r;
        \\  fragment =    vec4(luma, luma, luma, 1.0);
        \\  }
    ;
